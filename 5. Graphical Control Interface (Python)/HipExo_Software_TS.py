'''
Entry 05/06/2024
This is a modification of the runnig version of "HipExo_GUI_v11.py".

The following was added:
This one works with the torque sensors
...
L_motor_torquesensor_buffer = t_buffer.copy()
R_motor_torquesensor_buffer = t_buffer.copy()
...
class MainWindow(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        global t_buffer, L_IMU_buffer, R_IMU_buffer, BattVolt_buffer, L_motor_torque_buffer, R_motor_torque_buffer,\
            L_motor_torque_d_buffer, R_motor_torque_d_buffer, L_motor_torquesensor_buffer, R_motor_torquesensor_buffer,\
            t_0, t_minus_1,\
            tau_d, q_d,\
            ConnectButton, CmdButton, LoggingButton, Cmd_text, SerialComboBox,\
            Connection_Flag, connected_ports
                
        self.setWindowTitle('Hip Exoskeleton Software with Torque Sensors (TS) v1.0')
        ...
        L_Motor_TorqueSensor_plot = pg.PlotWidget()
        R_Motor_TnTd_plot         = pg.PlotWidget()
        R_Motor_TorqueSensor_plot = pg.PlotWidget()
        ...
        LMotorLayout.addWidget(L_Motor_TorqueSensor_plot) 

        # Rigth MotorLayout
        RMotorLayout.addWidget(R_Motor_TnTd_plot)
        RMotorLayout.addWidget(R_Motor_TorqueSensor_plot)
        ...
        # Position
        L_Motor_TorqueSensor_plot.setTitle("Torque sensor 1 (Left)", **title_style)
        L_Motor_TorqueSensor_plot.setLabel('left', "Torque [Nm]", **label_style)
        L_Motor_TorqueSensor_plot.setLabel('bottom', "Time [s]", **label_style)
        L_Motor_TorqueSensor_plot.setBackground('w')
        L_Motor_TorqueSensor_plot.showGrid(x=True, y=True)
        self.L_motor_torquesensor_line = L_Motor_TorqueSensor_plot.plot(t_buffer, L_motor_torquesensor_buffer, pen = red)
        ...
        # Position
        R_Motor_TorqueSensor_plot.setTitle("Torque Sensor 2 (Right)", **title_style)
        R_Motor_TorqueSensor_plot.setLabel('left', "Torque [Nm]", **label_style)
        R_Motor_TorqueSensor_plot.setLabel('bottom', "Time [s]", **label_style)
        R_Motor_TorqueSensor_plot.setBackground('w')
        R_Motor_TorqueSensor_plot.showGrid(x=True, y=True)
        self.R_motor_torquesensor_line = R_Motor_TorqueSensor_plot.plot(t_buffer, R_motor_torquesensor_buffer, pen = red)
    ...
    def update_plot_data(self):
        global t_buffer, L_IMU_buffer, R_IMU_buffer, BattVolt_buffer, L_motor_torque_buffer, R_motor_torque_buffer,\
            ...
            L_motor_torque_desired, R_motor_torque_desired, t_teensy, L_torque_sensor, R_torque_sensor,\
            t_0, t_minus_1,\
            tau_d, q_d,\
        ...
        if Connection_Flag == True:
            ...
            if t_minus_1 != t:
                ...
                L_motor_torquesensor_buffer = L_motor_torquesensor_buffer[1:]
                L_motor_torquesensor_buffer.append(L_torque_sensor)
                ...
                R_motor_torquesensor_buffer = R_motor_torquesensor_buffer[1:]
                R_motor_torquesensor_buffer.append(R_torque_sensor)
                ...
                self.L_motor_torquesensor_line.setData(t_buffer, L_motor_torquesensor_buffer)
                ...
                self.R_motor_torquesensor_line.setData(t_buffer, R_motor_torquesensor_buffer)
                ...
                if LogginButton_Flag == True:
                    LoggedData = {
                        ...
                        "L_TorqueSensor": L_torque_sensor,
                        "R_Torque_d": R_motor_torque_desired,
                        "R_Torque": R_motor_torque,
                        "R_TorqueSensor": R_torque_sensor
                    }
            ...
            if ser.read(1) == b'\xA5':  # 165 in uint8
                if ser.read(1) == b'\x5A':  # 90 in uint8
                    ...
                    if expected_length == bytes([ble_datalength]):  # Check the length
                        ...
                        L_torque_sensor        = decoded_data[7]
                        R_torque_sensor        = decoded_data[8]

                        print(str(L_torque_sensor) + " | " + str(R_torque_sensor))
...
def LogginButton_Clicked():
    ...
    DataHeaders = ["time", "L_IMU", "R_IMU", "L_Torque_d", "L_Torque", "L_TorqueSensor", "R_Torque_d", "R_Torque", "R_TorqueSensor"]
